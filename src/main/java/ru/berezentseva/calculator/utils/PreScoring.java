...
//    public static LocalDate birthdate = null;
 ...
 //   public static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd");
    ...
    private void validateTerm(int term) throws ScoreException {
            if (term < term_MIN) {
                throw new ScoreException("Срок кредита должен быть > = 6 месяцев. Введено значение: %s");
            }
        }
    ...
    private void validatePassport(String passportSeries, String passportNumber) throws ScoreException {
        if (!passportSeries.matches(passportSeries_REGEX)) {
            throw new ScoreException("Серия паспорта должна содержать 4 цифры. Введено значение: %s");
        }
        if (!passportNumber.matches(passportNumber_REGEX)) {
            throw new ScoreException("Номер паспорта должен содержать 6 цифр. Введено значение: %s");
        }
    }

    private void validateEmail(String email) throws ScoreException {
        if (!email.matches(email_REGEX)) {
            throw new ScoreException("Некорректный формат email. Введено значение: %s");
        }
    }
